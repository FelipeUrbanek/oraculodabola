...
export async function generateCelebrationImage(m: Milestone): Promise<string> {
    ...
    <html>
      ...
      <body>
        <div style="width:1080px;height:1350px;" class="relative overflow-hidden flex flex-col items-center justify-center">
          <div class="absolute inset-0 z-0">${namesHtml}</div>
          ...
          <div class="relative z-50 flex flex-col items-center">
            ...
            <p class="text-3xl font-light tracking-[0.4em] uppercase text-center max-w-[80%] opacity-70">${m.sub}</p>
          </div>
        </div>
      </body>
    </html>
  ...
  const outputDir = path.join(process.cwd(), 'posts', date);
